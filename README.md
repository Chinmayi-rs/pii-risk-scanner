...
**Related project:** I later built a [PII Redaction Tool](https://github.com/Chinmayi-rs/pii-redaction-tool) 
as a follow-up. Where this project detects and risk-scores PII, that one actually remediates 
it (via redaction or reversible tokenization).
...
- **ReDoS (Regular Expression Denial of Service) mitigation:** all detector regex patterns 
  use bounded quantifiers (e.g. `{8}`, `{13,19}`) rather than unbounded ones, avoiding the 
  nested-quantifier structures that cause catastrophic backtracking. As an additional 
  safeguard against untrusted or malformed input, each detector function enforces a maximum 
  cell length (1000 characters) before running any regex match. Cells exceeding this are 
  skipped rather than scanned, preventing excessive processing time on abnormally large or 
  adversarial input.
- This tool was developed and tested against synthetic, well-formed data (via `Faker`). 
  Real-world datasets may contain messier, larger, or differently-formatted values. The 
  length cap above is a first line of defense, but a production deployment would benefit 
  from additional hardening (e.g. regex execution timeouts via the `regex` library, input 
  encoding validation, and file-size limits before loading data into memory).
